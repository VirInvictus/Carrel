...
- [x] MED, CI never exercises the fork-side icon gate (checkout is this-repo-only, so the runner always prints the skip) while README/CLAUDE attribute the check to CI and misdescribe it as palette closure (it is a byte-diff to logo.svg). Add the sibling checkout (stated coupling trade-off) + scope the doc claims; bump actions v5 to v7.
      *(Executed 2026-09-15: the action bumps landed (checkout and setup-python to v7), README/CLAUDE/ci.yml scope the claims honestly, and the coupling gate came back GO: CI checks the fork out beside this repo (CARREL_FORK), so the icon byte-diff runs there and a fork icon drift reddens Carrel CI by design. The rendered pair stays local (no render tools on the runner; the script prints its skip).)*
- [x] MED, GitHub batch (recorded, grown): zero Releases behind 13 tags (cut v0.9.7/0.9.8/0.9.9 verbatim; note the v0.9.7 body would publish the recorded em-dash, same gated call as the tag fix); README hotlinks none of the fork's six screenshots; description never mentions Carrel-calibre-web; topic swap open (drop docs/just/gpl-30); wiki on and never initialized.
      *(Executed 2026-09-15, batch GO: Releases cut from all three tags with verbatim bodies; the v0.9.7 em-dash question answered publish-as-is, so the tag record stands unrewritten. README gained the Screenshots section (raw URLs on the fork's smallscope branch, coupling stated), the description names the fork, topics swapped (docs/just/gpl-30 out; design-system/typography/reading-room in), wiki off.)*
...
- [x] LOW, Docs batch: roadmap.md:715 "below" pointer (the note is above); Phase 0 "(in flight)" and Phase 13 "(digging only)" headers stale against shipped reality and README:95; patchnotes errata (0.5.0's "CalibreQuarry's engine" + dead v2.6+ floor; 0.7.0's 154 figure); decide the historical-correction policy once; "decision 32/33" cite an unlinked private log; de-duplicate the four-place theme-guard restatement; spec spelling split (§8.3-8.5 British vs American elsewhere) + the curly-apostrophe paste seam; 12 spaced-hyphen dash surrogates in the two new-findings blocks; Soehne/Söhne; the check recipe omits the surprising guards; sync-theme's comment should carry the one-way warning at the write path.
      *(Executed 2026-09-15: pointer, both headers, both decision anchors, the 0.5.0 dated erratum, all 12 dash surrogates, the apostrophe seam (7 normalized), Soehne, the guard-restatement de-dup (README/CLAUDE/ci.yml point at the script docstring), the check-recipe comment, and the write-path warning. The two gates came back: the historical-correction policy is inline dated errata (the fork precedent), decided once, and the 0.7.0 154-figure entry carries its erratum under it; the spelling convention is Canadian English, recorded in the spec header, with the §8.3-8.5 sections and the stray American prose normalized.)*
- [x] LOW, Hygiene: .gitignore is ~45 lines of packaging-era entries this repo can never produce (taste call); bare *.bak unignored; .ruff_cache/ local cruft. Feature candidates logged (FINAL-REPORT L4): dropdown theming + notation closure + --mono guard + compound just sync + sheet sanity as one small release; derivative guard; report-only selector-vs-DOM linter; /basic skin note; the 1.0.0 agent-executable prep (workspace-map row + DNF marking; the six sign-off boxes stay Brandon's).
      *(Executed 2026-09-15: *.bak ignored, .ruff_cache removed, the .gitignore pruned to the lean set per the gate answer (library, editor/OS, and scratch blocks only), and every listed feature candidate shipped in this release (dropdown + notation closure + --mono guard + compound just sync + sheet sanity + derivative guard + the report-only linter + the /basic note). The workspace-map row is updated; the DNF marking stays a deliberate library write in the curation workflow, and the six 1.0 sign-off boxes are recorded reopen conditions in project.done.)*
