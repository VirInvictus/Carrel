...
Last revised: 2026-09-15 (v0.9.10). Companion repo: `Carrel-calibre-web`
...
Spelling: Canadian English (colour, -ize; licence the noun, license the
verb; "gray" in prose follows the CSS token names). Decided 2026-09-15;
code identifiers and historical release records are exempt.
...
- The generated recolour block and `scripts/recolor_caliblur.py` retire. There
...
- caliBlur's raster backgrounds (`blur-light.png`, averaging `#3a4853`, a cool
  ...
  any colour-level override and were the single largest source of drift from the
  palette below.
...
and unusable for "colour equals category". Measured against the categorical
...
**Rule: no categorical colour anywhere.** Where a chart shows more than one
...
1. **Covers carry the colour.** Surfaces stay in the near-black dragonBlack
...
`scripts/recolor_caliblur.py`: roughly 178 rules whose caliBlur colours were
...
- **Rasters are unreachable.** caliBlur's page background is a PNG, so no
  colour-level generator could ever retheme it.
...
- **Notation coverage is a trap.** The generator matched `#hex` only, so
  colours written as `rgba()` passed through un-themed and rules containing no
...
- Badge colours: Read = dragonGreen, Reading = dragonBlue2, To Read =
...
labels for identity, never colour alone.
...
recognizes only `"` (`search_query_parser.py:158`), so `#Audience:'Rin'`
